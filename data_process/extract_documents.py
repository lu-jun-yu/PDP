...
从文书原文 (.txt) 中，基于规则提取结构化字段，输出 JSON。
...
Usage:
    python data_process/extract_documents.py
    python data_process/extract_documents.py --input-dir data/public_sources/Stage1 --output-dir data/public_sources/Stage1
...
def extract_id(text: str) -> str:
    """从文书正文提取案号作为 ID。"""
    m = re.search(r"[\u4e00-\u9fff]+[〔\[]\d{4}[〕\]][A-Za-z]?\d+号", text)
...
def parse_document(filepath: str, doc_type: str = "non_prosecution") -> dict | None:
    ...
    # ----- 元数据 -----
    # 案号在正文中（body 已去掉网页来源噪声行），不在原始页面附加信息中
...
def main():
    parser = argparse.ArgumentParser(
        description="从文书原文（不起诉决定书 + 起诉书）中提取结构化 JSON"
    )
    parser.add_argument(
        "--input-dir", default="data/public_sources/Stage1",
        help="输入根目录，递归查找 起诉/不起诉 目录 (default: data/public_sources/Stage1)",
    )
    parser.add_argument(
        "--output-dir", default="data/public_sources/Stage1",
        help="输出根目录 (default: data/public_sources/Stage1)",
    )
    parser.add_argument(
    ...
    if args.split_date:
        ...
        # ---- 默认：按目录结构划分 ----
        stage1_has_flat_dirs = any(
            os.path.isdir(os.path.join(args.input_dir, name))
            for name in ("起诉", "不起诉")
        )
        if stage1_has_flat_dirs:
            _, stats = process_split(
                args.input_dir,
                os.path.join(args.output_dir, "dataset.json"),
                "Stage1",
                limit=args.limit,
            )
