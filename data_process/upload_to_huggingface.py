...
将本地 pdp10k 数据集上传到 HuggingFace Hub。

前置条件:
    1. 先运行 convert_to_hf_dataset.py 生成 data/pdp10k
    2. 项目根目录下需有 token.json（含 hf_token 字段）或通过 --token 参数传入

Usage:
    python upload_to_huggingface.py --repo-id your-username/pdp10k
    python upload_to_huggingface.py --repo-id your-username/pdp10k --private
    python upload_to_huggingface.py --repo-id your-username/pdp10k --token hf_xxxxx
"""
...
# PDP-10K: Prosecution Decision Prediction Dataset

PDP-10K 是一个中文检察起诉决定预测数据集，包含约 10,000 条结构化的检察文书数据。

## 数据集结构

| Split | 样本数 | 起诉 | 相对不起诉 | 法定不起诉 | 存疑不起诉 |
|-------|--------|------|-----------|-----------|-----------|
| train | 8,192  | 4,096 | 2,048 | 1,024 | 1,024 |
| test  | 2,048  | 1,024 | 512   | 256   | 256   |
...
| `fact` | string | 案件事实 |
| `relevant_articles` | list[string] | 相关法条（带前缀 cl:/cpl:/cpr:） |
...
def main():
    parser = argparse.ArgumentParser(
        description="将 pdp10k 数据集上传到 HuggingFace Hub"
    )
    parser.add_argument(
        "--dataset-dir", default="data/pdp10k",
        help="本地数据集目录 (default: data/pdp10k)",
    )
    parser.add_argument(
        "--repo-id", required=True,
        help="HuggingFace 仓库 ID，如 your-username/pdp10k",
    )
    parser.add_argument(
        "--token", default=None,
